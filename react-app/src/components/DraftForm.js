    ...
    <Paper className={classes.paper}>
      ...
      <form noValidate onSubmit={handleSubmit}>
        ...
        >
          ...
          <Select value={tournament} onChange={updateTournament} label='Tournament Year'>
            {availableTournaments.length ? (
              availableTournaments.map(tourney => (
                <MenuItem key={`availableTournaments-${tourney.id}`} value={tourney}>
                  {tourney.year}
                </MenuItem>
              ))
            ) : (
        ...
        <Button type='submit' fullWidth variant='contained' color='primary' disabled={!tournament}>
          Create Draft
        </Button>
      </form>
    </Paper>
