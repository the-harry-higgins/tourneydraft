...
import { Button, Grid, Paper, TextField, Typography, InputLabel, MenuItem, FormControl, Select } from '@material-ui/core';
    ...
    <Grid className={classes.grid} container spacing={3}>
      <Grid item xs={12} md={6}>
        <Paper className={classes.paper}>
          <Typography variant='h2' color='primary'>Join a League</Typography>
          <form
            noValidate
            onSubmit={handleJoinLeagueSubmit}>
            <FormControl
              variant="outlined"
              margin="normal"
              color='primary'
              ...
              <Select
                ...
                onChange={updateJoinLeagueName}
                label="League Name"
              >
                {joinableLeagues.length ?
                  joinableLeagues.map(league => (
                    <MenuItem
                      key={`joinableLeague-${league.id}`}
                      value={league.name}>
                      {league.name}
                    </MenuItem>
                  )) :
      ...
      <Grid item xs={12} md={6}>
        <Paper className={classes.paper}>
          <Typography variant='h2' color='secondary'>Create a New League</Typography>
          <form
            noValidate
            onSubmit={handleNewLeagueSubmit}>
            <TextField
              color='secondary'
              variant="outlined"
              margin="normal"
