...
const order = (thisRound, teams) => {
  ...
  } else if (teams[0].games_by_round[thisRound - 1] > teams[1].games_by_round[thisRound - 1]) {
    teams.reverse();
  }
  return teams;
};
...
export default function Game(props) {
  ...
  const colors =
    props.game.winning_team_id === orderedTeams[0].id
      ? [classes.winner, classes.loser]
      : [classes.loser, classes.winner];
