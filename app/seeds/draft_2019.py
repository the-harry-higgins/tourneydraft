...
def seed_draft_2019():
    ...
    draft = Draft(league_id=league.id, tournament_id=tournament.id,
    ...
    draft_order = generate_draft_order(league.id)
    draft = Draft(league_id=league.id, tournament_id=tournament.id,
                  draft_order=json.dumps(draft_order), draft_index=64,
                  draft_time=datetime.now(),
                  drafting=False, current_drafter_id=None)
    db.session.add(draft)
    db.session.commit()
    ...
    for i in range(0, 64):
        pick = random.choice(tuple(march_madness_team_ids))
...
def undo_draft_2019():
    db.session.execute(
        'DELETE FROM drafts USING tournaments WHERE drafts.tournament_id = tournaments.id AND tournaments.year = 2019;')
    db.session.commit()
